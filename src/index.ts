			...
			if (message.type == "getContent" && await joplin.settings.value('myBacklinksCustomSettingAutoMarkdown'); ) {
				...
				while (has_more) {
					...
					for (let i = 0; i < notes.items.length; i++) {
						let element = notes.items[i];
						//references = references + "\n" + `[${escapeTitleText(element.title)}](:/${element.id})`;
						references = references + "<br>" + `<a href="#" onclick="webviewApi.postMessage('${contentScriptId}', {type:'openNote',noteId:'${element.id}'})">${escapeTitleText(element.title)}</a><br>`;
					if (notes.has_more) { page = page + 1 } else { has_more = false }
				...
				let response = `<b>Backlinks!!</b><br>${newData}`
				return response;
